<?php
	class ContentLoader {
        ...
        public function load() {
            ...
            $this->controller->set("info_system_software", SW_VER);
            $this->controller->set("info_system_date", date('D M j H:i:s T Y'));
            $this->controller->set("info_system_uptime", shell_exec("uptime -p"));
            $this->controller->set("info_system_hostname", shell_exec("hostname"));
            
            
            //Load CPU Information
            $this->controller->set("info_cpu_processor", str_replace("Processor	:", "", shell_exec("cat /proc/cpuinfo | grep 'Processor' | uniq")));
            $this->controller->set("info_cpu_architecture", shell_exec("arch"));
            $load_avg = shell_exec("uptime | sed -n -e 's/^.*e: //p'");
            $this->controller->set("info_cpu_avg", $load_avg);
            $this->controller->set("info_cpu_avg_per", round(array_sum(explode(",", $load_avg))/3*100));
            ...
            $this->controller->set("info_cpu_bogomips", $aux);
            ...
            $this->controller->set("info_ram_total", $mem[1]);
            $this->controller->set("info_ram_use", round($mem[2]/$mem[1]*100));
            $this->controller->set("info_ram_used", $mem[2]);
            
            $swap = shell_exec("free -m | grep Swap");
            $swap = str_replace("Swap:", "", $swap);
            $swap = explode(" ", preg_replace("!\s+!", " ", $swap));
            $this->controller->set("info_swap_total", $swap[1]);
            $this->controller->set("info_swap_use", round($swap[2]/$swap[1]*100));
            $this->controller->set("info_swap_used", $swap[2]);
            ...
            $this->controller->set("info_storage", $aux);
            ...
            foreach($interfaces as $interface) {
                ...
                if($enabled == "true") {
                    ...
                    $enabled = "<a class=\"waves-effect waves-light btn green btn-action\" data-action=\"interface_enable\" data-interface=\"$name\"><i class=\"material-icons left\">wifi_tethering</i>enable</a>";
            ...
            $this->controller->set("info_interfaces", $aux);
        }
    }
