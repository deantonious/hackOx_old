<?php
	...
	class View {
		...
		private function renderPage() {
			...
			include(ROOTPATH . "/views/" . $this->controller);
		...
		private function renderModulePage() {
			...
			//Module View 
			$controller = new Template("modules/$module_directory/" . $this->template);
			include(ROOTPATH . "modules/$module_directory/" . $this->controller);
			...
			$log_file = ROOTPATH . "modules/$module_directory/$module_directory.log";
			if(file_exists($log_file)) {
				$page->set("module_logs", "<pre>".file_get_contents($log_file)."</pre>");
			} else {
